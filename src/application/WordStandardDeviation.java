...
public class WordStandardDeviation implements MapReduce {  
	...
	public void reduce(FormatReader reader, FormatWriter writer) {
		...
		WordStandardDeviation.nbWords+=hm.size();
	 ...
	 private static double readAndFindMean(Format reader) {
		 	...
		 	KV kv; 
			while ((kv = reader.read()) != null) {
				acc += Double.parseDouble(kv.k) * Double.parseDouble(kv.v);
				acc_2 += Math.pow(Double.parseDouble(kv.k), 2) * Double.parseDouble(kv.v);
				sumFreq += Double.parseDouble(kv.v);
		    }
			
		    double mean = acc/sumFreq;
		    double temp = acc_2/sumFreq;
		    
		    System.out.println("mean=" + mean);
		    return Math.sqrt(temp-Math.pow(mean, 2));
	 } 
	...
	public static void main(String args[]) {
		...
		double stdDeviation = readAndFindMean(reader);
		System.out.println("Voici la valeur de l'écart type = " + stdDeviation + " ! ");
